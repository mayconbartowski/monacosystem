      ...
      <div className="p-4 md:p-6 bg-surface-sunken flex-1 overflow-auto space-y-2">
        <Card className="surface-card p-4 flex items-start gap-3">
          ...
          <div className="text-xs text-muted-foreground">
            ...
            Atendimento, Lava-jato e Gerente. Você pode alterar o login de qualquer conta. A{" "}
            <span className="text-foreground font-medium">senha</span> só pode ser alterada pelo usuário que está logado
            (sua própria senha).
          </div>
        </Card>
...
function AccountEditor({ account, onSaved }: { account: AppAccount; onSaved: () => void }) {
  ...
  return (
    <Card className="surface-card p-5 space-y-4">
      ...
      <Button
        onClick={save}
        disabled={busy || login === account.username || login.trim().length < 3}
